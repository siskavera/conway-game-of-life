...
class TestInit():
    ...
    def test_initial_state(self):
        ...
        test_ca = cellular_automaton.CellularAutomaton(initial_state=initial_state)
...
@patch.object(cellular_automaton, "get_next_state")
class TestGetAttractor():
    def test_fixed_point(self, mock_get_next_state):
        test_state = numpy.zeros([2, 3])
        test_ca = cellular_automaton.CellularAutomaton(initial_state=test_state)
    ...
    def test_attractor_cycle(self, mock_get_next_state):
        test_ca = cellular_automaton.CellularAutomaton(initial_state=numpy.zeros([2, 2]))
...
def test_evolve_simple(mock_get_next_state):
    ...
    test_ca = cellular_automaton.CellularAutomaton(initial_state=expected_states[0])
